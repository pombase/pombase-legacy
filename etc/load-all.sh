...
$POMBASE_CHADO/script/pombase-import.pl $POMBASE_LEGACY/load-pombase-chado.yaml generic-cvtermprop \
    ...
    --separator-char="," --ignore-missing-cvterms \
...
curl -s $POMBASE_PREDICTION_URL | perl -ne 'print unless /\tC\t/' > $SOURCES/pombase-prediction.gaf.new || echo failed to download pombase-prediction.gaf
...
gzip -d < $GOA_POMBE_AND_JAPONICUS |
    $POMBASE_CHADO/script/pombase-import.pl ./load-pombase-chado.yaml gaf --use-only-first-with-id \
       ...
       --term-id-filter-filename=$SOURCES/pombe-embl/goa-load-fixes/filtered_GO_IDs \
       --with-filter-filename=$SOURCES/pombe-embl/goa-load-fixes/filtered_mappings \
       --assigned-by-filter=GOC,InterPro,UniProtKB,UniProt,RHEA,IntAct,RNAcentral,CAFA,CACAO \
...
$POMBASE_CHADO/script/pombase-export.pl ./load-pombase-chado.yaml interactions --organism-taxon-id=4896 "$HOST" $DB $USER $PASSWORD | gzip -9 > $CURRENT_BUILD_DIR/$DB.interactions.tsv.gz
...
psql $DB -c "\COPY (SELECT pub.uniquename AS pmid, base_db.name || ':' || base_x.accession AS termid, t.name, p.value AS COMMENT FROM pombase_feature_cvterm_ext_resolved_terms fc JOIN feature_cvtermprop p ON fc.feature_cvterm_id = p.feature_cvterm_id JOIN cvterm t ON t.cvterm_id = fc.cvterm_id JOIN cvterm pt ON pt.cvterm_id = p.type_id JOIN pub ON fc.pub_id = pub.pub_id JOIN cvterm base_t ON fc.base_cvterm_id = base_t.cvterm_id JOIN dbxref base_x ON base_x.dbxref_id = base_t.dbxref_id JOIN db base_db ON base_db.db_id = base_x.db_id WHERE pt.name = 'submitter_comment' ORDER BY pub.uniquename, p.value) TO STDOUT DELIMITER E'\t' CSV HEADER;" > $CURRENT_BUILD_DIR/logs/$log_file.annotation_comments.tsv
...
RUST_BACKTRACE=full $POMCUR/bin/pombase-chado-json -c $POMBASE_WEB_CONFIG \
   ...
   --community-reviewers $POMBE_EMBL/supporting_files/gocam-community-reviewers.tsv \
   --gocam-model-directory $POMBE_EMBL/supporting_files/gocam-py-noctua-models 2>&1 | tee $LOG_DIR/$log_file.web-json-write
...
then
    ...
    do
      duckdb -c "copy (select * from read_json('$CURRENT_BUILD_DIR/misc/public_api_${api_file_type}.json', sample_size=-1) order by all) to '$CURRENT_BUILD_DIR/exports/public_api_${api_file_type}.parquet' (FORMAT parquet, COMPRESSION zstd, COMPRESSION_LEVEL 12, PARQUET_VERSION V2);"
    done
