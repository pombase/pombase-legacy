...
then
    ...
    do
      duckdb -c "copy (select * from read_json('$CURRENT_BUILD_DIR/misc/public_api_${api_file_type}.json', sample_size=100000) order by all) to '$CURRENT_BUILD_DIR/exports/public_api_${api_file_type}.parquet' (FORMAT parquet, COMPRESSION zstd, COMPRESSION_LEVEL 12, PARQUET_VERSION V2);"
    done
